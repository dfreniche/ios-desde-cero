...
struct PixelArtListView: View {
    ...
    @State var deletedPixelArt = false

    var body: some View {
        NavigationView {
            List($pixelArts) { pixelArt in
                NavigationLink(destination: deletedPixelArt ? PixelDrawingView(pixelArt: $pixelArts.first!) : PixelDrawingView(pixelArt: pixelArt)) {
                    // this is the "cell"
                    VStack {
